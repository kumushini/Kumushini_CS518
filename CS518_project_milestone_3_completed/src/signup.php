...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($firstname_err) && empty($lastname_err) && empty($useremail_err) && empty($password_err) && empty($confirm_password_err)) {
    	...
    	$rest_api_key = rand(100000,999999);

        // Prepare an insert statement
        $sql = "INSERT INTO user (email,first_name,last_name,password,rest_api_key) VALUES (?, ?, ?, ?,?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_useremail, $param_firstname, $param_lastname, $param_password,$param_rest_api_key);
            ...
            $param_rest_api_key = $rest_api_key;
